			...
			size_t count, loff_t *fPos)
{
	...
	pr_info("MY FINGERPRINT >>> vfsspi_read called");
	...
	if (spidev != NULL) {
		...
		if (!vfsspi_device->enabled_clk) {
			...
			if (spi_info != NULL) {
				...
				/* System LSI W/A Code */
				exynos7_update_media_scenario(TYPE_SPDMA, 1, 0);
...
{
	pr_info("MY FINGERPRINT >>> vfsspi suspend called.");
	pr_info("%s\n", __func__);
...
static int vfsspi_open(struct inode *inode, struct file *filp)
{
	...
	pr_info("MY FINGERPRINT >>> vfsspi_open called.");
...
static void vfsspi_shutdown(struct spi_device *spi)
{
	...
	pr_info("MY FINGERPRINT >>> vfsspi shutdown called.");
	if (g_data != NULL)
		vfsspi_disable_debug_timer();
...
static void __exit vfsspi_exit(void)
{
	...
	pr_info("MY FINGERPRINT >>> vfsspi exit called.");
