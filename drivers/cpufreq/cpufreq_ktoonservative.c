...
struct cpufreq_ktoonservative_tunables {
	...
	unsigned int freqs_raise_screen_on;
	unsigned int freqs_raise_screen_off;
	unsigned int freqs_lower_screen_on;
	unsigned int freqs_lower_screen_off;
...
show_one_kt(freqs_lower_screen_on);
show_one_kt(freqs_lower_screen_off);
show_one_kt(freqs_raise_screen_on);
show_one_kt(freqs_raise_screen_off);
...
#define store_lockout_core_soff(file_name, ndx)						\
static ssize_t store_##file_name							\
(struct cpufreq_ktoonservative_tunables *tunables, const char *buf, size_t count)	\
{											\
	unsigned int input;								\
...
store_lockout_core_soff(lockout_hotplug_screen_off_core_1, 1);
store_lockout_core_soff(lockout_hotplug_screen_off_core_2, 2);
store_lockout_core_soff(lockout_hotplug_screen_off_core_3, 3);
store_lockout_core_soff(lockout_hotplug_screen_off_core_4, 4);
store_lockout_core_soff(lockout_hotplug_screen_off_core_5, 5);
store_lockout_core_soff(lockout_hotplug_screen_off_core_6, 6);
store_lockout_core_soff(lockout_hotplug_screen_off_core_7, 7);
...
static ssize_t store_freqs_raise_screen_on(struct cpufreq_ktoonservative_tunables
		*tunables, const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	/* no need to test here if freqs_raise_screen_on is zero as the user might actually
	 * want this, they would be crazy though :) */
	tunables->freqs_raise_screen_on = input;
	return count;
}

static ssize_t store_freqs_raise_screen_off(struct cpufreq_ktoonservative_tunables
		*tunables, const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	/* no need to test here if freqs_raise_screen_off is zero as the user might actually
	 * want this, they would be crazy though :) */
	tunables->freqs_raise_screen_off = input;
	return count;
}

static ssize_t store_freqs_lower_screen_on(struct cpufreq_ktoonservative_tunables
		*tunables, const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	/* no need to test here if freqs_lower_screen_on is zero as the user might actually
	 * want this, they would be crazy though :) */
	tunables->freqs_lower_screen_on = input;
	return count;
}

static ssize_t store_freqs_lower_screen_off(struct cpufreq_ktoonservative_tunables
		*tunables, const char *buf, size_t count)
{
	unsigned int input;
	int ret;
	...
	/* no need to test here if freqs_lower_screen_off is zero as the user might actually
	 * want this, they would be crazy though :) */
	tunables->freqs_lower_screen_off = input;
	return count;
}
...
show_store_gov_pol_sys(freqs_raise_screen_on);
show_store_gov_pol_sys(freqs_raise_screen_off);
show_store_gov_pol_sys(freqs_lower_screen_on);
show_store_gov_pol_sys(freqs_lower_screen_off);
...
gov_sys_pol_attr_rw(freqs_raise_screen_on);
gov_sys_pol_attr_rw(freqs_raise_screen_off);
gov_sys_pol_attr_rw(freqs_lower_screen_on);
gov_sys_pol_attr_rw(freqs_lower_screen_off);
...
static struct attribute *dbs_attributes_gov_sys[] = {
	...
	&freqs_raise_screen_on_gov_sys.attr,
	&freqs_raise_screen_off_gov_sys.attr,
	&freqs_lower_screen_on_gov_sys.attr,
	&freqs_lower_screen_off_gov_sys.attr,
...
static struct attribute *dbs_attributes_gov_pol[] = {
	...
	&freqs_raise_screen_on_gov_pol.attr,
	&freqs_raise_screen_off_gov_pol.attr,
	&freqs_lower_screen_on_gov_pol.attr,
	&freqs_lower_screen_off_gov_pol.attr,
...
static const char *ktoonservative_sysfs[] = {
	...
	"ignore_nice_load",
	"freqs_raise_screen_on",
	"freqs_raise_screen_off",
	"freqs_lower_screen_on",
	"freqs_lower_screen_off",
	"debug_enabled",
};
...
int get_next_offline_small(void) {
	int x;
	for(x = 1; x < 4; x++) {
		if(!cpu_online(x) && hotplug_cpu_lockout[x] == 0) {
			return x;
		}
	}
	return -1;
...
static bool check_freq_increase(struct cpufreq_ktoonservative_cpuinfo *this_dbs_info, struct cpufreq_policy *policy, unsigned int max_load)
{
	...
	if ((screen_is_on && max_load > tunables->up_threshold_screen_on) || (!screen_is_on && max_load > tunables->up_threshold_screen_off)) {
		...
		{
			if (screen_is_on)
				//freq_target = (tunables->freqs_raise_screen_on * policy->max) / 200;
				freq_target = tunables->freqs_raise_screen_on;
			else
				//freq_target = (tunables->freqs_raise_screen_off * policy->max) / 200;
				freq_target = tunables->freqs_raise_screen_off;
			
			//pr_info("MY FREQ CHECK >>> Freq_target is: %d\n", freq_target);
			...
			//pr_info("MY FREQ CHECK >>> Requested freq is: %d for CPU: %d\n", this_dbs_info->requested_freq, policy->cpu);
			if (this_dbs_info->requested_freq > policy->max) {
				this_dbs_info->requested_freq = policy->max;
				//pr_info("WAS ABOVE >>> Requested freq was higher than max freq for cpu: %d\n", policy->cpu);
			}
...
static bool check_freq_decrease(struct cpufreq_ktoonservative_cpuinfo *this_dbs_info, struct cpufreq_policy *policy, unsigned int max_load)
{
	unsigned int freq_target;
	struct cpufreq_ktoonservative_tunables *tunables = policy->governor_data;
	if (((screen_is_on && max_load < (tunables->down_threshold_screen_on)) || (!screen_is_on && max_load < (tunables->down_threshold_screen_off - 10))))
	{
		if (screen_is_on)
			//freq_target = (tunables->freqs_lower_screen_on * policy->max) / 200;
			freq_target = tunables->freqs_lower_screen_on;
		else
			//freq_target = (tunables->freqs_lower_screen_off * policy->max) / 200;
			freq_target = tunables->freqs_lower_screen_off;
		
		//pr_info("MY FREQ DECREASE >>> Freq target is: %d\n", freq_target);

		this_dbs_info->requested_freq -= freq_target;
		if (this_dbs_info->requested_freq < policy->min)
			this_dbs_info->requested_freq = policy->min;

		//pr_info("MY FREQ DECREASE >>> Requested freq is: %d for CPU: %d\n", this_dbs_info->requested_freq, policy->cpu);
...
/* int get_avg_load(int cluster) {
	int load_val = 0, x, online = 0, avg = 0;
	
	switch (cluster) {
		case 0:
			for(x = 0; x < 4; x++) {
				load_val = cpu_load[x];
				//pr_info("CPU LOAD >>> CPU: %d has a load of %d\n", x, cpu_load[x]);
				if(cpu_online(x)) {
					avg += load_val;
					online++;
				}
	...
	//pr_info("MY AVG >> Returning avg of: %d with %d cpu online for cluster %d.\n", avg, online, cluster);
	return avg;

} */

static void dbs_kt_check_cpu(struct cpufreq_ktoonservative_cpuinfo *this_dbs_info)
{
	unsigned int load = 0;
...
//	unsigned int freq_target;
	...
	for (cpuLoop = loopStart; cpuLoop <= loopStop; cpuLoop++)
	{
		...
		if (!boostpulse_relayf || (boostpulse_relayf && !tunables->lockout_changes_when_boosting))
		{
			...
			if (!(should_boost_fingerprint && fingerprint_booster) && max_load <= hotplug_cpu_enable_down[cpuLoop] && hotplug_cpu_lockout[cpuLoop] != 1 && !tunables->disable_hotplug && !disable_hotplug_chrg_override && !disable_hotplug_media_override && !disable_hotplug_bt_active)
			{
				...
				{
					if (((screen_is_on && Lblock_cycles_offline[cpuLoop] > tunables->block_cycles_offline_screen_on) || (!screen_is_on && Lblock_cycles_offline[cpuLoop] > tunables->block_cycles_offline_screen_off)))
					{
						
						int new_cpu = get_next_online_big();
						if(cpuLoop < 4 && new_cpu != -1 && favor_small_cores) {
							//pr_info("MY HOT >>> Turning off big core instead of small. POLICY: %d\n", policy->cpu);
							if (max_load < cpu_load[0])
								max_load = cpu_load[0];
							//policy = cpufreq_cpu_get(0);
							//this_dbs_info->policy = policy;
							
							set_core_flag_down(new_cpu, 1);
							set_core_flag_up(new_cpu, 0);
							queue_work_on(0, dbs_wq, &hotplug_offline_work);
							Lblock_cycles_offline[new_cpu] = 0;
							goto freq_check_cpu;
						}
...
//exitSecondaries:
	...
	if ((screen_is_on && tunables->freqs_raise_screen_on == 0) || (!screen_is_on && tunables->freqs_raise_screen_off == 0))
		return;
	...
	if (!tunables->no_extra_cores_screen_off || screen_is_on)
	{
		for (cpu = loopStart; cpu <= loopStop; cpu++)
		{
			...
			if (!(should_boost_fingerprint && fingerprint_booster) && max_load >= hotplug_cpu_enable_up[cpu] && (!cpu_online(cpu)) && hotplug_cpu_lockout[cpu] != 2)
			{
				...
				{
					int new_cpu = get_next_offline_small();
					pr_info("MY HOT >>> First CPU value: %d\n", cpu);
					//pr_info("MY HOT >>> Will be bigger core? %d\n", (cpu >= 4 && new_cpu != -1 && favor_small_cores && !check_small_cores_online()));
					if(cpu >= 4 && new_cpu != -1 && favor_small_cores && !check_small_cores_online()) {
						pr_info("MY HOT >>> Turning on small core instead of big. GOT: %d\n", cpu);
						if (max_load < cpu_load[0])
							max_load = cpu_load[0];							
						set_core_flag_up(new_cpu, 1);
						set_core_flag_down(new_cpu, 0);
						this_dbs_info->Lblock_cycles_online = 0;
						//hotplug_flag_on = true;
						queue_work_on(0, dbs_wq, &hotplug_online_work);
						
					} else {
						pr_info("MY HOT >>> Turning on core: %d instead of small.\n", cpu);
						set_core_flag_up(cpu, 1);
	...
	//else
freq_check_cpu:
	if (!call_in_progress && ((screen_is_on && tunables->super_conservative_screen_on) || (!screen_is_on && tunables->super_conservative_screen_off)))
	{
...
void ktoonservative_screen_is_on(bool state, int cpu)
{
	...
	pr_info("MY SCREEN >>> Screen is on: %d for CPU: %d\n", state, cpu);
	if (pcpu)
	{
		if (pcpu->policy)
		{
	...
	if (state)
	{
		...
		if (!cpu_online(4) && hotplug_cpu_lockout[4] != 2)
		{
	...
	else
	{
		...
		for (cpuloop = 1; cpuloop < CPUS_AVAILABLE; cpuloop++)
		{
			pr_info("MY SCREEN >>> Is main_cpufreq_control[cpuloop]: %d\n", main_cpufreq_control[cpuloop]);
...
static void __cpuinit hotplug_offline_work_fn(struct work_struct *work)
{
	...
	for(cpu = 7; cpu > 0; cpu--) {
		if(cpu_online(cpu) && hotplug_cpu_lockout[cpu] != 1) {
			pr_info("MY OFFLINE >>> main_cpufreq_control[cpu] is: %d\n", main_cpufreq_control[cpu]);
			if((hotplug_cpu_single_down[cpu] && !hotplug_cpu_single_up[cpu] && !main_cpufreq_control[cpu]) || force_cores_down[cpu]) {
				if(cpu == 4) {
					if(hotplug_cpu_lockout[4] == 2) {
						cpu_down(5);
						set_core_flag_down(5, 0);
						set_core_flag_up(5, 0);
						cpu_down(6);
						set_core_flag_down(6, 0);
						set_core_flag_up(6, 0);
						cpu_down(7);
						set_core_flag_down(7, 0);
						set_core_flag_up(7, 0);
						cpu_down(4);
						set_core_flag_down(4, 0);
						set_core_flag_up(4, 0);
					}
				} else {
					cpu_down(cpu);
					set_core_flag_down(cpu, 0);
...
static void __cpuinit hotplug_online_work_fn(struct work_struct *work)
{
	...
	/* for_each_possible_cpu(cpu) {
		if (likely(!cpu_online(cpu) && (cpu))) {
			if (cpu <= 4 || (cpu >= 5 && cpu_online(4)))
			{
				if (hotplug_cpu_single_up[cpu] && !hotplug_cpu_single_down[cpu] && hotplug_cpu_lockout[cpu] != 2)
				{
					if (debug_is_enabled)
	...
	for(cpu = 0; cpu < 8; cpu++) {
		...
		//} else {
			//pr_info("MY HOT >>> Likely was false for cpu: %d\n", cpu);
		}
		if (hotplug_cpu_single_down[cpu])
			set_core_flag_down(cpu, 0);
	}
...
static void cpufreq_param_set_init(struct cpufreq_ktoonservative_tunables *tunables)
{
	...
	tunables->freqs_raise_screen_on = 50000;
	tunables->freqs_raise_screen_off = 32000;
	tunables->freqs_lower_screen_on = 80000;
	tunables->freqs_lower_screen_off = 100000;
