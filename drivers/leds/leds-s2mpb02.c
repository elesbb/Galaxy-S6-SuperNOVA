...
void set_custom_flashlight(struct i2c_client *client, const u8 reg, unsigned int brightness) {
	brightness = brightness - 2000;
	global_led_datas[S2MPB02_TORCH_LED_1]->data->brightness = brightness;
	led_set(global_led_datas[S2MPB02_TORCH_LED_1]);
	s2mpb02_write_reg(client, reg, brightness);
			...
			size_t count)
{
	...
	if (value == 0) {
		...
		set_custom_flashlight(global_led_datas[S2MPB02_TORCH_LED_1]->i2c, S2MPB02_REG_FLED_CUR1, value);
		return count;
	} else {
