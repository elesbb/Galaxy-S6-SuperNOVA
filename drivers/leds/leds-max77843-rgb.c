...
static u8 led_dynamic_current = 0xFF;

static u8 normal_powermode_current = 0xFF;
static u8 low_powermode_current = 0xFF;
...
unsigned int led_charger_on = 1000;
unsigned int led_charger_off = 1000;
unsigned int led_charger_mode = 0;
unsigned int led_enable_fade = 0;
unsigned int led_bright = 100;
...
void max77843_rgb_set_state_real(struct led_classdev *led_cdev,
...
static void max77843_rgb_set_state(struct led_classdev *led_cdev,
				unsigned int brightness, unsigned int led_state)
{
	brightness = (unsigned)(int)((led_bright * brightness) / 100);
	max77843_rgb_set_state_real(led_cdev, brightness, led_state);
	pr_info("LED BRIGHTNESS IS: %u\n", brightness);
	return;
	/* const struct device *parent = led_cdev->dev->parent;
	struct max77843_rgb *max77843_rgb = dev_get_drvdata(parent);
	struct device *dev;
				...
				unsigned int brightness, unsigned int led_state)
{
	...
	if(brightness != 0) {
		...
		case RED:
			brightness = brightness * brightness_ratio_r / 100;
			pr_info("LED WAS RED: %d\n", brightness);
			break;
		case GREEN:
			brightness = brightness * brightness_ratio_g / 100;
			pr_info("LED WAS GREEN: %d\n", brightness);
			break;
		case BLUE:
			brightness = brightness * brightness_ratio_b / 100;
			pr_info("LED WAS BLUE: %d\n", brightness);
			break;
		}
...
static int max77843_rgb_ramp(struct device *dev, int ramp_up, int ramp_down)
{
	...
	if (ramp_down <= 1000) {
		...
		ramp_down = (ramp_down - 1000) * 2 + 1000;
		ramp_down /= 100;
	}
					...
					const char *buf, size_t count)
{
	...
	led_r_brightness = (unsigned)(int)((led_bright * led_r_brightness) / 100);
	led_g_brightness = (unsigned)(int)((led_bright * led_g_brightness) / 100);
	led_b_brightness = (unsigned)(int)((led_bright * led_b_brightness) / 100); */
	...
	/*Set LED blink mode*/
	if(led_enable_fade) {
		if(delay_on_time < delay_off_time) {
			max77843_rgb_ramp(dev, delay_on_time, delay_on_time);
		} else {
			max77843_rgb_ramp(dev, delay_off_time, delay_off_time);
		}
	}
	max77843_rgb_blink(dev, delay_on_time, delay_off_time);
	pr_info("MY BRIGHT >>> delay on: %d delay off: %d\n", delay_on_time, delay_off_time);	
	return count;
}
...
static ssize_t led_bright_show(struct device *dev, struct device_attribute *attr, char *buf) {
	int ret;
	ret = sprintf(buf, "%d\n", led_bright);
...
static ssize_t led_bright_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count) 
{
	int retval;
	...
	pr_info("MY_VALUE >> retval: %d, enabled: %d\n", retval, enabled);
	if(retval != 0 && (enabled <= 255)) {
		led_bright = enabled;
	} else {
		led_bright = 0;
	}
...
static ssize_t led_chrg_mode_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	int ret;
	ret = sprintf(buf, "%d\n", led_charger_mode);
...
static ssize_t led_chrg_off_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	int ret;
	ret = sprintf(buf, "%d\n", led_charger_off);
...
static ssize_t led_chrg_on_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	int ret;
	ret = sprintf(buf, "%d\n", led_charger_on);
...
static ssize_t led_chrg_off_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	int retval;
	...
	led_charger_off = enabled;

	return count;
}

static ssize_t led_chrg_on_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	int retval;
	...
	led_charger_on = enabled;

	return count;
}

static ssize_t led_chrg_mode_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	int retval;
...
//Fade LED code
static DEVICE_ATTR(led_chrg_on, 0664, led_chrg_on_show, led_chrg_on_store);
static DEVICE_ATTR(led_chrg_off, 0664, led_chrg_off_show, led_chrg_off_store);
static DEVICE_ATTR(led_chrg_mode, 0664, led_chrg_mode_show, led_chrg_mode_store);
static DEVICE_ATTR(led_fade, 0664, led_fade_show, led_fade_store);
static DEVICE_ATTR(led_bright, 0664, led_bright_show, led_bright_store);
...
static struct attribute *sec_led_attributes[] = {
	...
	&dev_attr_led_bright.attr,
	&dev_attr_led_chrg_on.attr,
	&dev_attr_led_chrg_off.attr,
	&dev_attr_led_chrg_mode.attr,
