...
#if defined(CONFIG_AFC_CHARGER_MODE)
	dev_info(battery->dev,
		 "%s: Status(%s), mode(%s), Health(%s), Cable(%d), level(%d%%), HV(%s), Cycle(%d)\n",
		 __func__,
...
#else
	dev_info(battery->dev,
		 "%s: Status(%s), mode(%s), Health(%s), Cable(%d), level(%d%%), Cycle(%d)\n",
		 __func__,
		 sec_bat_status_str[battery->status],
		 sec_bat_charging_mode_str[battery->charging_mode],
		 sec_bat_health_str[battery->health],
		 battery->cable_type, battery->siop_level, battery->batt_cycle);
#endif
#if defined(CONFIG_SAMSUNG_BATTERY_ENG_TEST)
	dev_info(battery->dev,
...
skip_monitor:
	...
	if(bat_start_lvl) {
		if(battery->capacity <= bat_start_lvl) {
			should_apply_bat_mhz = 1;
		} else if(battery->capacity >= bat_stop_lvl && should_apply_bat_mhz) {
			should_apply_bat_mhz = 0;
		}
	}
	//my_battery_level = battery->capacity;
	//set_battery_level_core_ctrl(battery->capacity, battery->cable_type);
	/* for(i = 0; i < 7; i++) {
		pr_info("CORE CTRL >>> CPU Core: %d Level: %d\n", i, core_levels[i]);
		if(core_levels[i] > battery->capacity && !(battery->cable_type > 1 && disable_core_ctrl_chrgr)) {
			batt_lev_lockout[i] = 1;
		} else {
			batt_lev_lockout[i] = 0;
		}
	}
 */
	return;
